...
export default function EbookCard({ ebook }) {
  return (
    <div className="bg-[#1e293b] rounded-xl overflow-hidden border border-gray-800 hover:border-[#6366f1]/40 transition duration-300">
      ...
      {/* Cover Image */}
      <div className="h-56 bg-[#0f172a] flex items-center justify-center overflow-hidden">
        {ebook.coverImage ? (
          <img
      ...
      {/* Info */}
      <div className="p-4">
        <h3 className="text-white font-semibold text-sm mb-1 line-clamp-1">{ebook.title}</h3>
        <p className="text-gray-400 text-xs mb-2">by {ebook.writerName}</p>
        <p className="text-gray-500 text-xs mb-3 line-clamp-2">{ebook.description}</p>
        
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs bg-[#0f172a] text-gray-400 px-2 py-1 rounded-full">{ebook.genre}</span>
          <span className="text-white font-bold">${ebook.price}</span>
        </div>

        <div className="flex items-center justify-between mb-3">
          <span className="text-gray-500 text-xs">Sales: {ebook.sales || 0}</span>
