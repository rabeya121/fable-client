...
import { RiBookOpenLine, RiArrowLeftLine, RiHeartLine, RiHeartFill } from "react-icons/ri";
  ...
  if (!ebook) {
    return (
      <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center text-white">
        ...
        <Link href="/browse" className="bg-[#6366f1] hover:bg-[#4f46e5] text-white px-6 py-3 rounded-lg transition">
          Browse Ebooks
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] px-4 py-10">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Back Button */}
        <Link href="/browse" className="flex items-center gap-2 text-gray-400 hover:text-white text-sm mb-8 transition">
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <button onClick={handleBookmark} className="text-2xl text-[#6366f1] hover:scale-110 transition">
            ...
            <p className="text-gray-400 mb-2">
              by{" "}
              <Link href={`/browse?writer=${ebook.writerEmail}`} className="text-[#6366f1] hover:underline">
                {ebook.writerName}
              </Link>
            </p>

            <div className="flex flex-wrap gap-3 mb-6">
              <span className="bg-[#6366f1]/20 text-[#6366f1] px-3 py-1 rounded-full text-sm">{ebook.genre}</span>
            ...
            <p className="text-gray-300 leading-relaxed mb-8">{ebook.description}</p>

            <div className="flex items-center gap-6 mb-8">
              <span className="text-4xl font-bold text-white">${ebook.price}</span>
              <span className="text-gray-500 text-sm">{ebook.sales || 0} sold</span>
            </div>

            {/* Purchase Button */}
            {isWriter ? (
              <button disabled className="bg-gray-700 text-gray-400 px-8 py-3 rounded-xl font-semibold cursor-not-allowed">
                You are the writer
              </button>
            ) : (
              <button
